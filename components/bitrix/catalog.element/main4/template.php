...
$this->addExternalCss(SITE_TEMPLATE_PATH."/js/photoswipe/photoswipe.css");
$this->addExternalCss(SITE_TEMPLATE_PATH."/js/photoswipe/default-skin/default-skin.css");
$this->addExternalJS(SITE_TEMPLATE_PATH."/js/photoswipe/photoswipe.js");
$this->addExternalJS(SITE_TEMPLATE_PATH."/js/photoswipe/photoswipe-ui-default.min.js");
function isTestEnv(){
    $pos = strripos($_SERVER['HTTP_HOST'], 'test');
    if ($pos === false) {
        return false;
    } else {
        return true;
						...
						<ul class="pswp-ul">
							<?foreach($arResult["MORE_PHOTO"] as $i => $arImage){
								...
								<li id="photo-<?=$i?>" <?=(!$i ? 'class="current"' : 'style="display: none;"')?>>
									<?if(!$isEmpty){?>
										<a href="<?=($viewImgType=="POPUP" ? $arImage["BIG"]["src"] : "javascript:void(0)");?>" <?=($bIsOneImage ? '' : 'data-fancybox-group="item_slider"')?> class="<?=($viewImgType=="POPUP" ? "popup_link fancybx" : "line_link");?>" title="<?=$title;?>" data-size-w="<?=$arImage["WIDTH"]?>" data-size-h="<?=$arImage["HEIGHT"]?>" data-id="<?=$i?>">
					...
					<?if($arResult["MORE_PHOTO"]){
						foreach($arResult["MORE_PHOTO"] as $i => $arImage){?>
							...
							<li id="mphoto-<?=$i?>" <?=(!$i ? 'class="current"' : 'style="display: none;"')?>>
								...
								?>
								<?if(!$isEmpty){?>
									<a href="<?=$arImage["BIG"]["src"]?>" data-fancybox-group="item_slider_flex" class="fancybx popup_link" title="<?=$title;?>" >
		...
		<div class="info_item">
			...
			<?$frame = $this->createFrame()->begin();?>
				...
				<div class="prices_block">
                    ...
                    <div class="b-price-from"><?=$arResult['PROPERTIES']['PRICE_FROM']['NAME']?></div>
                    <?endif?>
						...
						<?}else{?>
							...
							if(isset($arResult['PRICE_MATRIX']) && $arResult['PRICE_MATRIX']) // USE_PRICE_COUNT
							{
								if($arResult['PRICE_MATRIX']['COLS'])
								{
									...
									<div class="creditprice" style="display:none"><?=($arResult['MIN_PRICE']['DISCOUNT_VALUE'] ? $arResult['MIN_PRICE']['DISCOUNT_VALUE'] : $arResult['MIN_PRICE']['VALUE'])?></div>
								<?}?>
							...
							<?if($arAddToBasketData["ACTION"] !== "NOTHING"):?>
								<?if($arAddToBasketData["ACTION"] == "ADD" && $arAddToBasketData["CAN_BUY"] && $arParams["SHOW_ONE_CLICK_BUY"]!="N"):?>
									<div class="wrapp_one_click">
										<span class="btn btn-default white btn-lg type_block transition_bg one_click t4" data-item="<?=$arResult["ID"]?>" data-iblockID="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arResult["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)">
											<span><?=GetMessage('ONE_CLICK_BUY')?></span>
										</span>
									</div>
								<?endif;?>
							<?endif;?>
							<span class="btn btn-default  btn-lg type_block transition_bg  gocredit show_rate" data-tpl="от {rate} руб" style="width:100%">
...
<div class="tabs_section type_more">
	...
	<div class="tabs">
		...
		<div class="tab-content">
			...
			<?endif;?>
			<div class="tab-pane <?=(!($iTab++) ? ' active' : '')?>" id="charakteristik">
...
<? foreach ( $arResult["DISPLAY_PROPERTIES"] as $arpropcena ) {
							if ($arpropcena["NAME"] == "Метраж 1 упаковки (кв.м)"){
							$cenacena=$arResult['MIN_PRICE']['VALUE'];
							$metrazh=$arpropcena["DISPLAY_VALUE"];
							$cenaupak = 2.22*$cenacena;
								echo  "$cenaupak <br />";}
}
?>
